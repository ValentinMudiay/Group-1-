...
class Voting_PageNavi_Options_Page extends scbAdminPage {
	...
	function setup() {
		...
		$this->args = array(
			'page_title' => __( 'Pagination Settings', 'voting-contest' ),
	...
	function page_content() {
		$rows = array(
			...
			array(				
				'title' => __( 'Number Of Contestants Per Page', 'voting-contest' ).' '.'<div class="hasTooltip"></div>
				<div class="hidden">' . __( 'Mention number Of Contestants Per Page.', 'voting-contest' ) .'</div>',
				'type' => 'text',
				'name' => 'contestant_per_page',
				'extra' => 'size="50"',
				'desc' => ''
			),
		...
		include(WPVC_VIEW_PATH.'wpvc_common_settings_view.php');
		$get_action ='paginate';
		...
		echo html( 'h2 class="color_h2"', $this->args['page_title'] );
		echo '<div class="settings_content">'.$this->form_wrap( $out ).'</div>';
