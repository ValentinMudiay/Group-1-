...
function voting_wp_pagenavi( $args = array(),$term_ids = NULL) {
	...
	$pages_to_shwpvc_minus_1 = $pages_to_show - 1;
	$half_page_start = floor( $pages_to_shwpvc_minus_1/2 );
	$half_page_end = ceil( $pages_to_shwpvc_minus_1/2 );
	...
	if ( ( $end_page - $start_page ) != $pages_to_shwpvc_minus_1 )
		$end_page = $start_page + $pages_to_shwpvc_minus_1;

	if ( $end_page > $total_pages ) {
		$start_page = $total_pages - $pages_to_shwpvc_minus_1;
	...
	$out = $before . "<div class='wp-pagenavi wpvc_votes-pagination' id='".$term_ids."' >\n$out\n</div>" . $after;
...
function get_multipage_link( $page = 1 ) {
	...
	if ( 1 == $page ) {
		...
		elseif ( 'page' == get_option( 'shwpvc_on_front' ) && get_option('page_on_front') == $post->ID )
