<?php
if(!function_exists('wpvc_vote_log_view')){
    function wpvc_vote_log_view($voting_logs,$log_entries,$trans_navigation){
		//global $wpdb;
	...
	<div class="wrap">
        ...
        <?php echo wpvc_add_vote_log_settings(); ?>
		...
		<form id="form_voting_logs" name="form_voting_logs" method="POST" action="<?php echo admin_url().'admin.php?page=votinglogs'; ?>">
			<div class="tablenav top">
				...
				<div class="alignleft actions bulkactions">
					<select name="action">
					<option selected="selected" value="-1"><?php _e('Bulk Actions','voting-contest'); ?></option>
						<option value="delete"><?php _e('Delete','voting-contest'); ?></option>
					</select>
			...
			<table id="table" class="widefat manage-questions voting_logs">
			    <thead>
				    <tr>
					  ...
					  <th class="manage-column column-title sortable <?php echo $log_entries['order']; ?>" id="values" scope="col" title="Click to Sort"  style="width:10%;">
						...
						<th class="manage-column column-no_votes" id="no_votes" scope="col" title="Click to Sort"  style="width:8%;">
						    <?php _e('No.of Votes','voting-contest'); ?>
					    </th>
				...
				if ( is_super_admin() || is_admin() ) {
					...
					?>
						...
						<td class="author column-no_votes" align="center"><?php echo $logs->votes; ?></td>
				...
				<tr>
					...
					<select name="logs_per_page" id="logs_per_page">
						<?php foreach($logs_per_page as $key => $logs): ?>
							<?php $selected = ($key == $records_per_page)?'selected':''; ?>
	...
	<script type="text/javascript">
        jQuery(document).ready(function () {
            jQuery('#logs_per_page').change(function (){
                ...
                document.location.href = '?page=votinglogs&logs_per_page='+jQuery('#logs_per_page').val();
            });
