			...
			<tr>
				<td style="box-sizing:border-box;text-align:center;white-space:nowrap;width:150px;vertical-align:middle;">
					<a href="<?php echo esc_attr( $src ); ?>"
					   ...
					   target="_blank"
					   class="button button-primary"
					   onclick="if (<?php echo esc_attr( $check_export_access ); ?>) { return true; } else { return false; }"
					   style="display:block;"
					>
				...
				<td style="vertical-align:middle;">
					...
					<?php if ( 'Table' === $this->dbo_type ) { ?>
						...
						<script type="text/javascript">
							...
							jQuery(document).ready(function() {
								jQuery('#format_type_<?php echo esc_attr( $this->rownum ); ?>').on('change', function() {
									update_export_button_<?php echo esc_attr( $this->rownum ); ?>();
								});
								jQuery('#include_table_settings_<?php echo esc_attr( $this->rownum ); ?>').on('change', function() {
									update_export_button_<?php echo esc_attr( $this->rownum ); ?>();
								});
							});
						</script>
